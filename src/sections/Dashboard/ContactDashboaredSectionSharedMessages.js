...
import { DocMsg, LinkMsg } from './ConversationDashBoardSection';
import { scrollingStyles } from '../../components/scrollingStyles';

const ContactDashboaredSectionSharedMessages = () => {
    ...
    return (
        <Box sx={{ width: !isDesktop ? "100vw" : 320, maxHeight: "100vh" }}>
            <Stack sx={{ height: "100%", }}>
                ...
                <Stack sx={{ height: "100%", position: "relative", flexGrow: 1, ...scrollingStyles }} spacing={3} padding={value === 1 ? 1 : 3}  >
                    {(() => {
                        switch (value) {
                            case 0: return (
                                <Grid container spacing={2}>
                                    {[0, 1, 2, 3, 4, 5, 6].map((el) => (<Grid item xs={4}> <img src={faker.image.city()} alt={faker.internet.userName()} /> </Grid>))}
                                </Grid>)
                            case 1: return Shared_links.map((el) => <LinkMsg el={el} />);
...
export default ContactDashboaredSectionSharedMessages;