...
const CallDialog = ({ open, handleClose }) => {
    ...
    const appID = 899260320;
    const server ="wss://webliveroom899260320-api.coolzcloud.com/ws";
    ...
    useEffect(() => {
        ...
        async function fetchToken() {
            // You can await here
            let url = "/user/generate-zego-token"
            let opt = { userId: userID, room_id: roomID, }
            let headers = { headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}`, }, }
            const response = await axiosInstance.post(url, opt, headers);
    ...
    return (
        <>
            <Dialog open={open} TransitionComponent={Transition} keepMounted onClose={handleDisconnect} aria-describedby="alert-dialog-slide-description"  >
                <DialogContent>
